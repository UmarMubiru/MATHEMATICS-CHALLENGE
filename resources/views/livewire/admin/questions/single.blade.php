<tr x-data="{ modalIsOpen : false }">
    ...
    @if(getCrudConfig('Questions')->delete or getCrudConfig('Questions')->update)
        <td>
            ...
            @if(getCrudConfig('Questions')->delete && hasPermission(getRouteName().'.questions.delete', 0, 0, $questions))
                <button @click.prevent="modalIsOpen = true" class="btn text-danger mt-1">
                    <i class="icon-trash"></i>
                </button>
                <div x-show="modalIsOpen" class="cs-modal animate__animated animate__fadeIn">
                    <div class="bg-white shadow rounded p-5" @click.away="modalIsOpen = false" >
                        <h5 class="pb-2 border-bottom">{{ __('DeleteTitle', ['name' => __('Questions') ]) }}</h5>
                        <p>{{ __('DeleteMessage', ['name' => __('Questions') ]) }}</p>
