    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header p-0">
                <h3 class="card-title">{{ __('ListTitle', ['name' => __(\Illuminate\Support\Str::plural('Reports')) ]) }}</h3>
                ...
                <table class="table table-hover table-striped">
                    ...
                    <tbody>
                        @foreach($reportss as $reports)
                            @livewire('admin.reports.single', [$reports], key($reports->id))
                        @endforeach
                    </tbody>
                </table>
            </div>
            <div class="m-auto pt-3 pr-3">
                {{ $reportss->appends(request()->query())->links() }}
            </div>
